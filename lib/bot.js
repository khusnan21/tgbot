...
/// Start of function for invalid status 
  bot.onText(/\/status/, async msg => {
    // help message
    const msg_status = `
<b>Invalid command,</b>

<code>/status</code> should have a magnet link so i can check status of file🙂️
e.g: /status magnet:urx...this_is_magnet_link..... 

`;
    bot.sendMessage(msg.chat.id, msg_status, msg_buttons);
  });  
/// Start of function for invalid download 
  bot.onText(/\/download/, async msg => {
    // help message
    const msg_down = `
<b>Invalid command,</b>

<code>/download</code> ary aalsi ansaan send me a magnet link with download command🙂️
e.g: /download magnet:urx...this_is_magnet_link..... 
⚠️ Make sure torrent has good seeders
🚀 more seeders means fast download speed
`;
    bot.sendMessage(msg.chat.id, msg_down, msg_buttons);
  });    
...
Seems you are muhey tang kerty 🙂️
To know available commands type /help


🚀 If you have added me in Group do not send too much links CPU garam ho jaye ga lol 
`;
    bot.sendMessage(msg.chat.id, msg_what, msg_buttons);
  });    
