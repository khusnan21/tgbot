...
//   });
  ...
  bot.onText(downloadRegex, (msg, match) => {
    ...
    const onDriveUploadStart = torr => bot.sendMessage(from, `📤 Uploading <code>${torr.name}</code> to a random stoarge...`, {parse_mode : "HTML"});
  ...
  bot.onText(statusRegex, (msg, match) => {
    ...
    if (link.indexOf("magnet:") !== 0) {
      ...
      if (!torr.done) {
        ...
        mess1 += `<b>You have the 🔗 Download Link & dont ask me if you lost - ulu ke pathay 😌️</b>\n\n`;
      }
      mess1 += `<b>Magnet URI 🧲:</b> <code>${torr.magnetURI}</code>`;
