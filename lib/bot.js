...
function bot(torrent, bot) {
  bot.onText(/\/start/, async msg => {
    // start message
    const startMessage = `
Hi ${msg.from.first_name} 😉️, <b>Here is what i can do 👇️</b>

 - <code>Search torrents from 1337x & piratebay</code>
 - <code>Download torrents for you</code>
 - <code>Save your torrent on Cloud</code>
 - <code>Give you download link valid for few days</code>
 - <code>If you loved my bot 😉️ follow me on twitter.com/gr8mawiya</code>

⚠️ Search may not work
⚠️ Beta: Now you can add to Groups (Admin)
...
<code>/start</code> - Start Me if sleeping 🙂️
...
<code>/search </code> - To Search a torrent from 2 sites
...
//     }
    try {
      ...
      const statusMessge = bot.sendMessage(chatId, "<code>📤 Uploading Your File to Cloud...</code>", {parse_mode : "HTML"});
      ...
      bot.sendMessage(chatId, `<b>💾 Uploaded to a random server</b> \n\n<b>Name 🏷️:</b> <code>${fileName}</code> \n\n<b>Backup Link 🔗:</b> ${driveLink} \n<b>Google Drive Link 🔗:</b> ${publicLink} \n\n `, {parse_mode : "HTML"});
      }
    } catch (e) {
  ...
  //  const driveLink = `https://drive.google.com/file/d/${driveId}/view?usp=sharing`;
    const reply = async torr => {
      ...
      if (!torr.done) {
        ...
        mess1 += `<b>Download Link 🔗: https://drive.google.com/file/d/${driveId}/view?usp=sharing </b>  \n\n`;
  ...
  bot.onText(statusRegex, (msg, match) => {
    ...
    if (link.indexOf("magnet:") !== 0) {
      ...
      if (!torr.done) {
        ...
        mess1 += `<b>Download Link 🔗:</b> <code> https://drive.google.com/file/d/${driveId}/view?usp=sharing </code>\n\n`;
      }
      mess1 += `<b>Magnet URI 🧲:</b> <code>${torr.magnetURI}</code>`;
